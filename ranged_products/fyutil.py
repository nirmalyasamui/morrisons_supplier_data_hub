...
class FYUtil:
    ...
    YTD_DATES_QUERY = 'WITH CURRENT_YTD as (SELECT  BSNS_YR_STRT_DT,DATE_TRUNC(CURRENT_DATE(), WEEK(MONDAY)) - 1 END_DT FROM `' + 'db_value' + '' + constant.TABLE_TIME_DAY + '` where EXTRACT(YEAR FROM current_date())  = EXTRACT(YEAR FROM BSNS_YR_STRT_DT) limit 1) SELECT CAST(CURRENT_YTD.BSNS_YR_STRT_DT as DATE) as YTD_START_DATE,CAST(CURRENT_YTD.END_DT as DATE) as YTD_END_DATE from CURRENT_YTD '
